...
std::stack<Pair> tracePath(cell cellDetails[][WIDTH], Pair dest)
{
    printf("\nThe Path is ");
    int row = dest.first;
    int col = dest.second;
    ...
    Path.push(std::make_pair(row, col));  //row->x->y
    ...
    while (!openList.empty()) {
        ...
        if (isValid(i - 1, j) == true) {
            ...
            if (isDestination(i - 1, j, dest) == true) {
                ...
                std::stack<Pair> path = tracePath(cellDetails, dest);
                foundDest = true;
                return path;
            }
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    for(i = 8; i < map.info.height; i++)
        ...
        {
            if(i >= map.info.width - 8 || j >= map.info.height - 8)
                continue;
            if(map.data[j + i*map.info.width] <= 55)
            {
              x = int((map.info.origin.position.y - global_map.info.origin.position.y)/global_map.info.resolution + i*map.info.resolution/global_map.info.resolution);
              y = int((map.info.origin.position.x - global_map.info.origin.position.x)/global_map.info.resolution + j*map.info.resolution/global_map.info.resolution);
    ...
    // int x = int((goal.y - global_map.info.origin.position.y)/global_map.info.resolution);
    // int y = int((goal.x - global_map.info.origin.position.x)/global_map.info.resolution);
    // Pair dest = std::make_pair(x, y);
    
    // x = int((rover_position.y - global_map.info.origin.position.y)/global_map.info.resolution);
    // y = int((rover_position.x - global_map.info.origin.position.x)/global_map.info.resolution);
    // Pair src = std::make_pair(x, y);

    // int grid[HEIGHT*WIDTH];
    // std::copy(global_map.data.begin(), global_map.data.end(), grid);

    // std::stack<Pair> path = aStarSearch(grid, src, dest);
    // nav_msgs::Path global_plan;
    // geometry_msgs::PoseStamped path_point;
    // path_point.header = global_map.header;
    // global_plan.header = global_map.header;


    // while (!path.empty()) {
    //     std::pair<int, int> p = path.top();
    //     path.pop();
    //     path_point.pose.position.x = p.second*RESOLUTION + global_map.info.origin.position.x;
    //     path_point.pose.position.y = p.first*RESOLUTION + global_map.info.origin.position.y;
    //     global_plan.poses.push_back(path_point);
    // }

    pub.publish(global_map);
    //pub_plan.publish(global_plan);
